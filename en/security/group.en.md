...
When a user belongs to multiple groups, the user's permission will take the maximum among the project-level, table-level and row-level permissions inherited from different groups.
...
The system administrator can manage user groups by adding a user to different groups in bulk, and deleting a user from multiple groups except for the group All Users. Once a user group is deleted, users in this group cannot be deleted, but all permissions granted to this group will be deleted.

#### Manage users in a user group

The system adminstrator can bulk add or delete users in a user group except for the group All Users. A user can be added to multiple groups.
...
Once LDAP is enabled, user group is read-only and cannot be added, edited, dropped, modified or assigned a user to.
